        ...
        {step === 1 && (
          <div className="w-full">
            <div className="max-w-4xl mx-auto text-center py-16 px-4">
              ...
              <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto px-4">
                ...
                <button type="button" onClick={() => { setFormData(prev => ({ ...prev, tipContract: 'auto' })); setStep(2); }} className="bg-[#12181D] border border-slate-700 text-white font-bold px-4 py-4 rounded-xl hover:border-[#8ba888]/50 transition text-xs tracking-tight flex items-center justify-center gap-2">
                    Asistent Inteligent Tranzacții Auto & RAR
                </button>
              </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
              ...
              {/* COLOANA 3: INTEROGARE RAR AUTO */}
              <div className="bg-[#12181D] p-5 rounded-2xl border border-slate-800 shadow-xl flex flex-col min-h-[380px] justify-between">
                <div className="w-full">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-2">Interogare Istoric Auto (RAR)</span>
                  <form onSubmit={handleQuickRarWidgetSubmit} className="space-y-1.5">
                    <input type="text" placeholder="Introdu serie șasiu (VIN)..." value={rarWidgetVin} onChange={e => setRarWidgetVin(e.target.value.toUpperCase())} className="w-full bg-[#0B0F12] border border-slate-700 rounded-lg px-2.5 py-1.5 text-xs text-white outline-none uppercase focus:border-[#8ba888]" />
                    <button type="submit" disabled={rarWidgetLoading} className="w-full bg-[#8ba888] text-[#0B0F12] text-xs font-bold py-1.5 rounded-lg transition hover:opacity-90">
                      {rarWidgetLoading ? 'Se interoghează...' : 'Verifică Istoric Vehicul'}
                    </button>
                  </form>
